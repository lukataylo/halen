...
package final class PluginRegistry {
    ...
    package func register(_ plugin: any HalenPlugin) {
    ...
    package func isEnabled(_ pluginId: String) -> Bool {
        enabledStates[pluginId] ?? readPersistedEnabled(pluginId)
    }

    package func toggle(_ pluginId: String) {
    ...
    /// Used by the "Remove" action for external plugins — the
    ...
    package func unregister(_ pluginId: String) {
        guard let plugin = plugins.first(where: { $0.id == pluginId }) else { return }
    ...
    package func contains(_ pluginId: String) -> Bool {
    ...
    /// immediate-value threshold: a brand-new user gets the Writing
    /// Assistant, Snippet Expander (which carries the folded-in
