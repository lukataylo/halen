...
package final class PluginHost {
    ...
    package init(services: HostServices) {
    ...
    package static var installRoot: URL {
    ...
    package func discoverManifests() -> [(URL, PluginManifest)] {
    ...
    package func spawn(at dir: URL, manifest: PluginManifest) async {
        guard !instances.contains(where: { $0.manifest.id == manifest.id }) else { return }
        // The plugin's *effective* capability set: declared in the manifest
        // AND not revoked in the permissions screen. `HostBridge` gates
        // every sensitive method on it.
        let granted = Set(services.broker.effectiveCapabilities(for: manifest).map(\.rawValue))
        ...
        let instance = PluginInstance(manifest: manifest, pluginDir: dir,
            ...
            case "hotkey/register", "hotkey/unregister":
                ...
                guard granted.contains(Capability.hotkeys.rawValue) else {
                    throw RPCErrorObject(
                        code: PluginRPC.ErrorCode.permissionDenied.rawValue,
                        message: "\(method) requires the `hotkeys` capability — declare it in halen-plugin.json",
                        data: nil)
                }
    ...
    package func terminate(id: String) async {
    ...
    package func startEventDispatcher() {
