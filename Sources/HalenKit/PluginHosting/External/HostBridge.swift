...
final class HostBridge {
    ...
    /// Backs the `ui/prompt` capability — the shared presenter so a second
    /// prompt supersedes the first rather than stacking popups.
    private var promptPresenter: PluginPromptPresenter { services.promptPresenter }

    init(services: HostServices) {
    ...
    func dispatch(method: String,
        ...
        case "inference/complete":
            // Ungated: shared model access is the platform's baseline.
            return try await inferenceComplete(params: params)
        case "ax/replaceRange":
            try require(Capability.insertText.rawValue, in: granted, for: method)
            return try await axReplaceRange(params: params)
        case "ax/readSelection":
            try require(Capability.observeText.rawValue, in: granted, for: method)
            return axReadSelection()
        case "ui/toast":
            try require(Capability.notifications.rawValue, in: granted, for: method)
            return uiToast(params: params)
        case "ui/prompt":
            try require(Capability.popoverUI.rawValue, in: granted, for: method)
            return await uiPrompt(params: params)
        case "clipboard/read":
            try require(Capability.clipboard.rawValue, in: granted, for: method)
            return clipboardRead()
        case "clipboard/write":
            try require(Capability.clipboard.rawValue, in: granted, for: method)
            return try clipboardWrite(params: params)
        case "calendar/upcomingEvents":
            try require(Capability.calendar.rawValue, in: granted, for: method)
        ...
        case "profile/getToneProfile":
            try require(Capability.toneProfiles.rawValue, in: granted, for: method)
    ...
    // MARK: - Clipboard (gated on the `clipboard` capability)

    private func clipboardRead() -> RPCValue {
        .object(["text": NSPasteboard.general.string(forType: .string)] as [String: Any?])
    }

    private func clipboardWrite(params: RPCValue?) throws -> RPCValue {
        guard let text = params?.objectValue?["text"]?.stringValue else {
            throw RPCErrorObject(code: PluginRPC.ErrorCode.invalidParams.rawValue,
                                 message: "clipboard/write requires `text`", data: nil)
        }
    ...
    /// Throw `permissionDenied` unless `capability` is in the caller's
    /// effective grant set (declared in the manifest and not revoked in the
    /// permissions screen). Every capability is opt-in per plugin — that's
    /// the platform's whole brand.
    private func require(_ capability: String,
        ...
        guard granted.contains(capability) else {
            throw RPCErrorObject(
                ...
                message: "\(method) requires the `\(capability)` capability — declare it in halen-plugin.json (and check it isn't revoked in Halen's permissions screen)",
                data: nil)
        }
    }
