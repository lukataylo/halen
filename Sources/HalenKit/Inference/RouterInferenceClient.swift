...
package actor RouterInferenceClient: InferenceClient {
    private let backends: [InferenceBackend]
    private let settings: InferenceSettings
    /// One gate per backend *instance*, index-aligned with `backends`.
    /// Keyed by position (not `BackendKind`) because two bundled-llama
    /// backends share a kind but hold independent model contexts — keying
    /// by kind used to serialize the Qwen classifier against Gemma rewrites
    /// for no reason.
    private let gates: [AsyncSemaphore]
    ...
    package init(backends: [InferenceBackend], settings: InferenceSettings) {
        self.backends = backends
        self.settings = settings
        self.gates = backends.map { _ in AsyncSemaphore(value: 1) }
    }

    /// The gate serializing requests to this backend instance. Backends are
    /// all classes/actors, so identity comparison is well-defined.
    private func gate(for backend: InferenceBackend) -> AsyncSemaphore {
        if let index = backends.firstIndex(where: { $0 as AnyObject === backend as AnyObject }) {
            return gates[index]
        }
        // Unreachable: every candidate chain is a filtered copy of `backends`.
        Log.error("Router: no gate for backend \(backend.kind.rawValue) — using first")
        return gates[0]
    }
    ...
    private func runStream(
        ...
        for backend in chain {
            ...
            let gate = self.gate(for: backend)
            do {
                // Priority-ordered acquire: a queued background request never
                // runs ahead of a user-initiated one. Throws if cancelled
                // while queued — no permit held.
                try await gate.wait(priority: request.priority)
            } catch {
    ...
    package func refreshAvailability() async {
    ...
    private func run(_ request: InferenceRequest, on backend: InferenceBackend) async throws -> InferenceResponse {
        let gate = self.gate(for: backend)
        // Priority-ordered acquire: a queued background request never runs
        // ahead of a user-initiated one. Throws `CancellationError` if
        // cancelled while queued — no permit was acquired in that case, so
...
package enum RouterError: Error, LocalizedError, CustomStringConvertible {
